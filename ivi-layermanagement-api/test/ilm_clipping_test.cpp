...
/*****************************************************************************
 * This test demonstrates the effect of shifting the position of a surface   *
 * outside of the containing layer. Surfaces and layers of the same          *
 * dimensional size are created. Surface destination rectangles are set to   *
 * be identical to the surface size. The layers are arranged in a            *
 * tessellated form exactly adjacent to each other.and then surfaces are     *
 * added so they perfectly overlap each layer. Each surface is positionally  *
 * shifted outside of the destination rectangle and layer specification.     *
 *                                                                           *
 * The surfaces parameters are checked to ensure that position, dimension    *
 * and destination rectangle are as expected. Layers and surfaces are        *
 * removed one by one, checking on each occasion that those remaining are    *
 * still valid.                                                              *
******************************************************************************/
...
/*****************************************************************************
 * This test demonstrates the effect of shrinking the destination rectangle  *
 * of a surface so that its overall dimension lies outside of it.            *
 * Surfaces and layers of the same dimensional size are created. The         *
 * position of each Surface is set to start at the same origin as layers.    *
 * The layers are arranged in a tessellated form exactly adjacent to each    *
 * other.and then surfaces are added so they perfectly overlap each layer.   *
 * The destination rectangles of each surface are set to have widths and     *
 * heights smaller than their respective surfaces width and height.          *
 *                                                                           *
 * The surfaces parameters are checked to ensure that position, dimension    *
 * and destination rectangle are as expected. Layers and surfaces are        *
 * removed one by one, checking on each occasion that those remaining are    *
...
/*****************************************************************************
 * This test demonstrates the effect of shrinking the layer which a surface  *
 * is associated with. Surfaces and layers of the same dimensional size are  *
 * created. The position of each Surface is set to start at the same origin  *
 * as layers. The layers are arranged in a tessellated form exactly adjacent *
 * to each other.and then surfaces are added so they perfectly overlap each  *
 * layer. The destination rectangles of each surface are set to have widths  *
 * and heights the same as their respective surfaces width and height.       *
 * The surfaces parameters are checked to ensure that position, dimension    *
 * and destination rectangle are as expected.                                *
 * The layer sizes are shrunk so that the surface dimensions and destination *
 * rectangle are outside of the layer boundaries.The surface parameters are  *
 * checked to ensure they are as expected.                                   *
 * The layer sizes are increased back to the original size. The surface      *
 * parameters are re-checked to confirm that they are as expected.           *
 * Layers and surfaces are removed one by one, checking on each occasion     *
 * that those remaining are still valid.                                     *
 *****************************************************************************/
...
/*****************************************************************************
 * This test demonstrates the effect of increasing the size of surfaces      *
 * destination rectangles beyond the dimensional size of containing layers.  *
 * Surfaces and layers of the same dimensional size are created.  The        *
 * position of each Surface is set to start at the same origin as its        *
 * respective layer. The layers are arranged in a tessellated form exactly   *
 * adjacent to each other and then surfaces are added so they perfectly      *
 * overlap each layer. The destination rectangles of each surface are set to *
 * have widths and heights the same as their respective surfaces width and   *
 * height.                                                                   *
 * The surfaces parameters are checked to ensure that position, dimension    *
 * and destination rectangle are as expected.                                *
 * The surface destination rectangles are modified so that their size is     *
 * greater than the associated layers boundary.The surface parameters are    *
 * checked to ensure they are as expected.                                   *
 * The surface destination rectangles are set back to the original size.     *
 * The surface parameters are re-checked to confirm that they are as         *
 * expected. Layers and surfaces are removed one by one, checking on each    *
 * occasion that those remaining are still valid.                            *
 *****************************************************************************/
...
/*****************************************************************************
 * This test demonstrates the effect of changing the orientation of surfaces *
 * so that part of the surfaces area is shifted outside of the associated    *
 * layer and the set destination rectangle.                                  *
 * Surfaces and layers of the same dimensional size are created (rectangular *
 * size with height greater than width). The position of each Surface is set *
 * to start at the same origin as its respective layer. The layers are       *
 * arranged in a tessellated form exactly adjacent to each other and then    *
 * surfaces are added so they perfectly overlap each layer. The destination  *
 * rectangles of each surface are set to have widths and heights the same as *
 * their respective surfaces width and height.                               *
 * The orientation of each surface is changed so that part of the surface is *
 * moved outside of the associated layer.                                    *
 * The surface parameters are checked to confirm that they are as expected.  *
 * The surface destination rectangle is increased to check if clipping is    *
 * affected.                                                                 *
 * The surface parameters are checked to confirm that they are as expected.  *
 * The surface destination rectangle is changed back and parameters checked. *
 * Layers and surfaces are removed one by one, checking on each occasion     *
 * that those remaining are still valid.                                     *
 *****************************************************************************/
...
/*****************************************************************************
 * This test demonstrates the effect of shifting the destination rectangle   *
 * of surfaces so that part of it is outside of the associated layer.        *
 * Surfaces and layers of the same dimensional size are created (rectangular *
 * size with height greater than width). The position of each Surface is set *
 * to start at the same origin as its respective layer. The layers are       *
 * arranged in a tessellated form exactly adjacent to each other and then    *
 * surfaces are added so they perfectly overlap each layer.                  *
 * The destination rectangle is set so that the surface destination is moved *
 * completely outside of the associated layer.                               *
 * The surface parameters are checked to confirm that they are as expected.  *
 * The surface destination rectangle is shifted back so that the destination *
 * rectangle is yet again contained within the associated layer area.        *
 * The surface parameters are checked to confirm that they are as expected.  *
 * The surface destination rectangle is changed back and parameters checked. *
 * Layers and surfaces are removed one by one, checking on each occasion     *
 * that those remaining are still valid.                                     *
