...
//        EXPECT_TRUE(callbackSurfaceId == (unsigned)-1 || callbackSurfaceId == surface);
    ...
    static bool SurfaceCallbackCheck(std::vector<t_ilm_surface>& callbacksRequested,
                                     std::vector<t_ilm_surface>& callbacksGot,
                                     uint line)
    {
        bool valid = (callbacksRequested == callbacksGot);

        EXPECT_EQ(valid, true)
            << "Surface callbacks don't match requested at line: "
            << line << std::endl;

        if (!valid)
        {
            std::cout << "Surface callback requested order: " << std::endl;

            for (uint i = 0; i < callbacksRequested.size(); i++)
            {
                std::cout << callbacksRequested[i] << std::endl;
            }

            std::cout << std::endl;

            std::cout << "Surface callback order got: " << std::endl;

            for (uint i = 0; i < callbacksGot.size(); i++)
            {
                std::cout << callbacksGot[i] << std::endl;
            }
        }
...
TEST_F(IlmCommandMultiClientTest, multi_Test) {
    uint no_surfaces = 4;
    uint no_layers = 2;
    std::vector<t_ilm_surface> callbacksRequested;
    ...
    for (uint i = 0; i < surfaces_allocated.size(); i++)
    {
        ...
        callbacksRequested.push_back(surfaces_allocated[i].returnedSurfaceId);
    }

    // Expect correct number of callback to have been called
    assertCallbackcalled(surfaces_allocated.size());

    SurfaceCallbackCheck(callbacksRequested, surface_callbacks, __LINE__);

    // Clear callbacks registered
    surface_callbacks.clear();
    callbacksRequested.clear();
    ...
    for (uint i = 0; i < num_surfaces; i++)
    {
        ...
        for (uint j = 0; j < length; j++)
        {
            ...
            if (index != num_surfaces)
            {
                ...
                callbacksRequested.push_back(surfaceIDs[j]);
